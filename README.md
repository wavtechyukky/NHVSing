...
**torch経路も既定で省メモリにしました（2026-09-21・ウェイトは不変）。** 同じ問題はtorch側にもあり、下の「使い方（V3）」の `NHVVocoder`（`nhv_vocoder.py`）が通る `dsp.py` のeager実装は37秒入力でピーク +5〜8GB、`export.py` の `FullVocoderV3`（ONNX用部品をeagerで実行するもの）は +4.2GB でした。対策は次の2点で、どちらも一括版と**ビット一致**します:

- 励起インパルス: 位相のcumsumだけ全長で取り、cosと倍音和は時間ブロックごとのPythonループで作る（`generate_impulse_train` の既定・32768サンプル固定。`impulse_train_onnx` のeager実行も同じ方式で、こちらは16384サンプル・環境変数 `NHV_IMP_BLOCK`）。
- 時変FIRとケプストラム→IR: フレームを256本ずつ処理し（`ltv_filter.frame_block` または環境変数 `NHV_FRAME_BLOCK`）、OLAは結合後に全長で1回。加算順序が変わらないので出力は同じです。

`NHVVocoder.infer` の出力は修正前と同一（7秒・37秒で最大差 0）です。全長のバッファ（フレーム化した信号やIRなど）は残るのでピークは入力長に比例して増えますが、傾きは約150〜280MB/秒から約9MB/秒になり、37秒で +0.5GB、240秒でも +2.3GB です。全長展開が消えたぶん速くもなります（37秒で 1.8〜3.1秒 → 1.2〜1.3秒。`FullVocoderV3` は同一セッションの前後比較で1/2/4/8スレッド −11% / −18% / −26% / −41%）。**学習は1ビットも変わりません**: クロップが32768サンプル以下なら励起は1ブロック、フレーム数がブロック長以下なら時変FIRも一括なので、従来と同じ計算を通ります（V3レシピの372msクロップはピッチ拡張で伸びても最大18176サンプル・71フレーム。`forward_train` の出力と勾配が修正前後で一致することを64・71フレームで確認済み）。`vocoder.harm_block` は不要になりましたが互換のため残しています。ONNXの書き出し経路（`Scan`）は変えていません（修正後に書き出したONNXは配布済みのものとグラフ・出力とも同一）。
...
**RTF**（Real-Time Factor = 音声1秒あたりの計算秒数。小さいほど速く、< 1 で実時間より速い）。測定環境はM4 MacBook Airの10コアCPU（高性能4＋高効率6）/ 約5秒入力 / バッチ1 / 9回の中央値。**各表の値はそれぞれ同一セッションで交互に測ったものです**（ORTセッションを同一プロセスに複数作ると1.4倍ほどブレるため、1プロセス1測定。NSF-HiFiGAN比較は2026-09-19、torch比較は上記eager修正後の2026-09-21の測定）。
...
多コアでは **torchの方が自前ONNXより速い**（4スレッドで約1.9倍、8スレッドで約2.5倍）という逆転が起きます。極小・FFT主体のモデルゆえ、ORTのグラフ最適化よりもtorchのバッチFFT並列化のほうが効くためです。したがって「NHVSingは遅い/速い」は単一の数字では語れず、**ランタイムとコア数の組み合わせで決まります**。

> このtorch列は `export.py` の `FullVocoderV3`（ONNX用部品をそのままeagerで実行したもの）で、学習側の `model.py::NHVSingV3` ではありません。ORT列も同じ日に同じセッションで測り直した値なので、上のNSF-HiFiGAN比較表のORT列とは測定日の違いぶん数%ずれます。
