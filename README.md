...
**torch経路（`export.py` の `FullVocoderV3`）も同じ問題をeagerで踏んでいました（2026-09-21修正・ウェイトは不変）。** 励起インパルスのeager実行は `Scan` 版をそのまま回しており、37秒入力でピーク +4.2GB でした。eagerでは時間方向に16384サンプルずつ（`NHV_IMP_BLOCK`）Pythonの `for` で回すようにし、一括版と**ビット一致**のままピークを +0.4GB にしています。ONNXの書き出し経路（`Scan`）は変えていません（修正後に書き出したONNXは配布済みのものとグラフ・出力とも同一）。逐次ループが消えたぶんeagerは速くもなります（同一セッションでの前後比較: 1/2/4/8スレッドで −11% / −18% / −26% / −41%）。
